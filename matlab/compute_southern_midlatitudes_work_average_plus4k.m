...
% (60S to 30S) of mechanical work and lift work, averaged over time.
...
% Select southern midlatitude region indices:
% latitude: 60S to 30S, all longitudes.
lat_idx = find(lat >= -60 & lat <= -30);
lon_idx = 1:length(lon);

if isempty(lat_idx)
    error('No grid points found for southern midlatitude bounds.');
end
fprintf('Southern midlatitudes selected: lat [%g, %g], all longitudes\n', ...
...
if all(lat_weights == 0)
    error('Latitude weights are all zero in the selected southern midlatitude region.');
end
...
fprintf('\n');
fprintf('=== RESULTS ===\n');
fprintf('Region: Southern Midlatitudes (60°S to 30°S, all longitudes)\n');
...
fprintf('\nMechanical Work (averaged over southern midlatitudes with weighted time mean): %.6f\n', work_avg);
fprintf('Lift Work (averaged over southern midlatitudes with weighted time mean): %.6f\n', lift_avg);
...
xlabel(plot_time_label); ylabel('Work'); title('Mechanical Work - Southern Midlatitudes Average Time Series');
grid on;

subplot(3, 1, 2);
plot(plot_time, lift_spatial_avg, 'r-', 'LineWidth', 1.5);
xlabel(plot_time_label); ylabel('Lift'); title('Lift Work - Southern Midlatitudes Average Time Series');
grid on;

subplot(3, 1, 3);
plot(plot_time, ratio_spatial_avg, 'k-', 'LineWidth', 1.5);
xlabel(plot_time_label); ylabel('Lift/Work'); title('Lift/Work Ratio - Southern Midlatitudes Average Time Series');
ylim([0 1])
grid on;

% Save figure
% savefig('./southern_midlatitudes_work_analysis.fig');
% fprintf('\nFigure saved as: southern_midlatitudes_work_analysis.fig\n');
