...
if [[ "$PRATE_THRESHOLDED" == "1" ]]; then
  ...
  if [[ "$LAT_BAND_THRESHOLDED" == "1" ]]; then
    ...
    for ((ip=0; ip<nthreshold; ip++)); do
      ...
      for ((ilat=1; ilat<=18; ilat++)); do
        ...
        if [[ "$ilat" -lt 18 ]]; then
          ncap_expr+="p${ip}=p${ip}+(((${lat_var}>=${south})&&(${lat_var}<${north}))*${precip_var}*(${precip_var}>=${tval}));"
          ncap_expr+="c${ip}=c${ip}+(((${lat_var}>=${south})&&(${lat_var}<${north}))*(${precip_var}>=${tval}));"
        else
          ncap_expr+="p${ip}=p${ip}+(((${lat_var}>=${south})&&(${lat_var}<=${north}))*${precip_var}*(${precip_var}>=${tval}));"
          ncap_expr+="c${ip}=c${ip}+(((${lat_var}>=${south})&&(${lat_var}<=${north}))*(${precip_var}>=${tval}));"
        fi
  ...
  # Collect remapped daily files once, then concatenate once.
  remapped_files=()
  ...
  # Concatenate all remapped daily files once (time axis).
  concat_all="$TMP_DIR/concat_all_thresholds.nc"
  echo "Concatenating ${#remapped_files[@]} remapped files once before percentile extraction"
  if ! ncrcat -O "${remapped_files[@]}" "$concat_all" 2>&1; then
    echo "Error: ncrcat failed for concatenated remapped files" >&2
    exit 1
  fi
  rm -f "${remapped_files[@]}"

  # For each threshold: extract p/c variables from the single concatenated file.
  # p* is already a daily conditional mean from the native-grid daily step.
  per_thresh_concat=()
  for ((ip=0; ip<nthreshold; ip++)); do
    pval="${pvals_arr[$ip]}"
    echo "Threshold $((ip+1))/$nthreshold: extracting p${ip}, c${ip} (percentile=$pval)"

    concat_thresh="$TMP_DIR/concat_thresh_${ip}.nc"
    if ! ncks -O -v "p${ip},c${ip},time,lon,lat" "$concat_all" "$concat_thresh" 2>&1; then
      echo "Error: ncks extraction of p${ip},c${ip} failed from concatenated file" >&2
      exit 1
    fi
  ...
  # Fix time calendar attribute to suppress CDO warnings
  ncatted -O \
