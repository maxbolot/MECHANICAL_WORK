...
case "$SIMULATION" in
    control)
        ...
        TARGET_DIR_COMPUTE_BASE=${TARGET_DIR_COMPUTE_BASE:-/scratch/gpfs/mbolot/results/GLOBALFV3/work_coarse_C3072_1440x720_prate_thresholded}
        THRESHOLD_FILE=${THRESHOLD_FILE:-$PROJECT_ROOT/output/thresholds/$RUN_ID/thresholds_control.txt}
        ;;
    warming)
        ...
        TARGET_DIR_COMPUTE_BASE=${TARGET_DIR_COMPUTE_BASE:-/scratch/gpfs/mbolot/results/GLOBALFV3/work_coarse_C3072_1440x720_PLUS_4K_CO2_1270ppmv_prate_thresholded}
        THRESHOLD_FILE=${THRESHOLD_FILE:-$PROJECT_ROOT/output/thresholds/$RUN_ID/thresholds_warming.txt}
        ;;
    *)
...
if [[ -z "${SLURM_ARRAY_TASK_ID:-}" ]]; then
    ...
    echo "Submitting thresholded async job array with $n_tasks tasks for simulation $SIMULATION"
    echo "Resources per array task: ntasks=$NTASKS, cpus-per-task=$CPUS_PER_TASK, mem-per-cpu=$MEM_PER_CPU"
        ...
        cat << EOF | python3 "$MANIFEST_TOOL" --output "$manifest_path" --workflow-type work --run-id "$RUN_ID" --project-root "$PROJECT_ROOT" --launcher "launcher/compute_work_async_prate_threshold_array.sh" --mode "slurm_array" --root "$TARGET_DIR_COMPUTE_BASE"
{
...
EOF
    ...
    sbatch --array=1-"$n_tasks" --ntasks="$NTASKS" --cpus-per-task="$CPUS_PER_TASK" --mem-per-cpu="$MEM_PER_CPU" \
        --output="$LOG_DIR/compute_work_async_prate_threshold_%A_%a.out" --error="$LOG_DIR/compute_work_async_prate_threshold_%A_%a.err" \
...
if [[ ! -x "$COMPUTE_BIN" ]]; then
    echo "Error: thresholded async binary not found or not executable." >&2
...
config_file="$namelist_dir/config_prate_work_${date}_${SLURM_ARRAY_TASK_ID:-0}_$$.nml"
cat << EOF > "$config_file"
&config
