...
if [[ ! -x "$COMPUTE_BIN" ]]; then
    echo "Error: lat-band thresholded compute binary not found or not executable." >&2
...
process_list() {
    ...
    while IFS= read -r raw_line; do
        ...
        config_file="$NAMELIST_DIR/config_prate_work_by_lat_band_${date}.nml"
        cat > "$config_file" << EOF
&config
...
EOF
        ...
        date_log="$LOG_DIR/compute_work_async_prate_threshold_by_lat_band_${date}.log"
        echo "[$(date +%F\ %T)] Running lat-band thresholded compute for date $date (log: $date_log)" | tee -a "$RUN_LOG"
