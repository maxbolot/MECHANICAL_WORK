...
if [[ -z "${SLURM_ARRAY_TASK_ID:-}" ]]; then
    ...
    echo "Submitting lat-band thresholded async job array with $n_tasks tasks for simulation $SIMULATION"
    echo "Resources per array task: ntasks=$NTASKS, cpus-per-task=$CPUS_PER_TASK, mem-per-cpu=$MEM_PER_CPU"
        ...
        cat << EOF | python3 "$MANIFEST_TOOL" --output "$manifest_path" --workflow-type work --run-id "$RUN_ID" --project-root "$PROJECT_ROOT" --launcher "launcher/compute_work_async_prate_threshold_by_lat_band_array.sh" --mode "slurm_array" --root "$TARGET_DIR_COMPUTE_BASE"
{
...
EOF
    ...
    sbatch --array=1-"$n_tasks" --ntasks="$NTASKS" --cpus-per-task="$CPUS_PER_TASK" --mem-per-cpu="$MEM_PER_CPU" \
        --output="$LOG_DIR/compute_work_async_prate_threshold_by_lat_band_%A_%a.out" --error="$LOG_DIR/compute_work_async_prate_threshold_by_lat_band_%A_%a.err" \
                --export=ALL,PROJECT_ROOT="$PROJECT_ROOT",SIMULATION="$SIMULATION",LIST_FILE_PART1="$LIST_FILE_PART1",LIST_FILE_PART2="$LIST_FILE_PART2",NTASKS="$NTASKS",CPUS_PER_TASK="$CPUS_PER_TASK",MEM_PER_CPU="$MEM_PER_CPU",RUN_ID="$RUN_ID",LOG_DIR="$LOG_DIR",MANIFEST_DIR="$MANIFEST_DIR",MANIFEST_TOOL="$MANIFEST_TOOL",TARGET_DIR_COMPUTE="$TARGET_DIR_COMPUTE",THRESHOLD_FILE="$THRESHOLD_FILE" "$SCRIPT_PATH"
    exit $?
fi
...
COMPUTE_BIN=${COMPUTE_BIN:-$PROJECT_ROOT/local/bin/compute_work_async_prate_threshold_by_lat_band}
if [[ ! -x "$COMPUTE_BIN" && "$COMPUTE_BIN" == "$PROJECT_ROOT/local/bin/compute_work_async_prate_threshold_by_lat_band" ]]; then
    COMPUTE_BIN=$PROJECT_ROOT/bin/compute_work_async_prate_threshold_by_lat_band
fi

if [[ ! -x "$COMPUTE_BIN" ]]; then
    echo "Error: lat-band thresholded async binary not found or not executable." >&2
    echo "Tried: $PROJECT_ROOT/local/bin/compute_work_async_prate_threshold_by_lat_band and $PROJECT_ROOT/bin/compute_work_async_prate_threshold_by_lat_band" >&2
    exit 1
fi
...
config_file="$namelist_dir/config_prate_work_by_lat_band_${date}_${SLURM_ARRAY_TASK_ID:-0}_$$.nml"
cat << EOF > "$config_file"
&config
    ...
    path_work_out   = '$target_dir_compute/work_prate_threshold_by_lat_band_${date}.nc',
