...
cat << EOF | python3 "$MANIFEST_TOOL" --output "$manifest_path" --workflow-type work --run-id "$RUN_ID" --project-root "$PROJECT_ROOT" --launcher "launcher/compute_work_async_prate_threshold_noslurm.sh" --mode "noslurm" --root "$TARGET_DIR_COMPUTE_BASE"
{
